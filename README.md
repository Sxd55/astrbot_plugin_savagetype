...
当前版本 `v5.6.0`。仓库：https://github.com/Sxd55/astrbot_plugin_savagetype
...
| 问题 | 本插件的做法 |
| --- | --- |
| 配置项太多（150+ 项），新手无从下手 | **开箱即用场景预设**：提供 `daily`（日常陪伴）、`frugal`（省Token）、`assistant`（知识助手）、`rpg`（沉浸扮演）4套成熟方案，一键切换；专家仍可选 `custom` 自由微调 |
| 多个插件抢着接话造成重复回复 | **协同握手避让**：自动探测 `savagereply` 等活跃中枢，检测到接管时主动避让，杜绝撞车 |
...
- **诊断**：Savage 生态全景健康体检 (Doctor)、Prompt 提示词注入实时预览、活跃群聊与消息流实时监控、注入显微镜（路由、命中、过滤原因、`chars≈tokens`）、聊天导入、回收站（归档+被覆盖恢复，槽位冲突阻止）、原始 JSON 诊断、清空并重建（先自动备份）。
- **设置**：151 项配置按左右分栏展示（左侧导航、右侧只显示选中的一组，未保存的输入切组不丢；保存按钮固定在右下；窄屏导航变为顶部横向条）——总开关与主人 / 免@接话 / 指派发言 / 消息防抖 / 记忆注入 / 隐私、画像与跨会话 / 事件与历史 / 抽取与整理 / 学习与表达 / 重要性与维护 / 模型与预算 / 图片 / 外观。支持顶部一键呼出场景预设与参数变更清单（Diff）弹窗。
...
| `/stype preset [diff|apply] [档位]` | 场景预设：查看当前生效、比对变更清单、一键应用预设（管理员/主人） |
