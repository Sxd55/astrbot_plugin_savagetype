...
当前版本 `v3.4.20`。仓库：https://github.com/Sxd55/astrbot_plugin_savagetype
...
1. QQ 消息进入时间线，任何新面孔自动建空档案。ChatUI（webchat）只记录主人自己的对话（进主人记忆），不建人物档案、不记录其他人；配置 `owner_qq` 后 ChatUI 记录会归到主人 QQ 名下。图片消息至少记一条 `[图片]` 占位，配了转述模型就记「[图片] 转述」。
...
4. **偏好按主题分槽**：喜欢猫、喜欢狗、喜欢咖啡三条共存；同一主题的改口（「不喜欢咖啡了」，句尾语气词会自动归一）才覆盖旧条。**身份归一**：配置 `owner_qq` 后，ChatUI 的主人会自动合并到主人 QQ 身份，历史记录一起迁移；合并时同槽留最新，升级迁移也会消解历史同槽冲突。
5. 同一人格、同一说话人、同一主题槽发生冲突时：新的生效，旧的归档；高证据旧事实被单次非纠正说法挑战时先进「待确认覆盖」。玩笑、转述、不确定不会覆盖。
6. 一条消息里的多个独立事实会拆成多条（「喜欢美式，不喜欢拿铁」= 两条；「我喜欢猫，我喜欢狗」也会拆成两条）。
7. **记忆生命周期**：基础重要性（主人手动 1.0 / AI 审核通过 0.8 / 未审核 0.5）按半衰期衰减，被召回会拉长半衰期；权重低于阈值且超期的记忆在维护时归档进「回收站」，置顶记忆永不归档。
8. **注入去重**：同一会话刚注入过的记忆，在去重窗口内不重复注入（记录落库，重载插件不丢）；「还记得 / 上次」类问题豁免。约定类记忆单独打成【约定】块，近况类打成【近况】块。
9. 注入包走 `req.extra_user_content_parts`，并 `mark_as_temp()`，不改 `system_prompt`，避免打爆前缀缓存；内容用「不可信数据」声明包裹，防止记忆里的文本被当指令执行。超预算时核心事实按行尽量塞，黑话 / few-shot / 草稿整块丢。
...
- `ui_theme_color` / `ui_theme_color2` / `ui_theme_color3`：面板三色主题（主色当 UI 强调，三色一起驱动背景着色器）。设置页有 5 组预设：极光、碧金、暮霞、午夜、森林；也可以三个取色器自定义。
